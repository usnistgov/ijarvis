...
<button type="button" class="btn btn-primary" onclick="Function_poscar_conv()">Show POSCAR-conv</button>
<script>
...
<button type="button" class="btn btn-primary" onclick="Function_xyz()">Show XYZ format</button>
<script>
...
function Function_cif() {
  ...
  var x=<xsl:value-of select="basic_info/main_relax_info/cif"/>;
  document.getElementById("contcar").style.display = 'block';
  document.getElementById("contcar").value = (x);
}
</script>
...
<!-- <button type="button" class="btn btn-primary" onclick="Function_rdf()">Show RDF data</button> -->
<!-- <script> -->
<!-- function Function_rdf() { -->
...
<!-- <button type="button" onclick="Function_xyzz()">Show all XML data</button> -->
<!-- <script> -->
<!-- function Function_xyzz() { -->
 
  <!-- var x=String(<xsl:value-of select="*"/>); -->
  <!-- document.getElementById("contcar").style.display = 'block'; -->
  <!-- document.getElementById("contcar").value = (x); -->
<!-- } -->
<!-- </script> -->
...
//Plotly.newPlot('bands', data,layout_convg,{displaylogo: false});
  ...
  var x= <xsl:value-of select="basic_info/main_spillage_info/nonsoc_bands/spin_down_bands_x"/>;
  x=x.split(';');
  
  var y= <xsl:value-of select="basic_info/main_spillage_info/nonsoc_bands/spin_down_bands_y"/>;
  y=y.split(';');
...
<!--Download files table starts--> 

 <script>
  var x= <xsl:value-of select="basic_info/main_relax_info/download_files"/>;
  if (x!==''){
   
   var header = document.createElement("h3");
   header.className="jcentered";
   var text = document.createTextNode("Download raw input/output files");
    header.appendChild(text);
    header.style.textAlign='center';
    document.body.appendChild(header);
    
             var divElement = document.createElement("table");
    divElement.id = "downloads";
    divElement.setAttribute('class', 'jcentered');
    document.body.appendChild(divElement);   
      var tmp=x.split(';');
       for (var i=0;i&lt;tmp.length-1;i++) {
        url=tmp[i].split(',');
        newlink = document.createElement('a');
        newlink.innerHTML = url[0];
        newlink.setAttribute('title', url[0]);

        newlink.setAttribute('href', url[2]);
        newlink.style.textAlign='center';
        document.body.appendChild(newlink);
